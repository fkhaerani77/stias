...
import { Calendar, ChevronLeft, ListChecks, Plus, Trash2 } from 'lucide-react-native';
...
export default function QuestionsCategoryScreen() {
  ...
  const activeCategory = activeCategoryId ? categories.find((c: any) => c.id === activeCategoryId) : null;
  ...
  const handleSubmit = () => {
    ...
    if (isNaN(durationNumber) || durationNumber <= 0) {
      Alert.alert('Durasi Tidak Valid', 'Durasi harus berupa angka menit yang valid.');
      return;
    }
  ...
  const handleDelete = () => {
    ...
    Alert.alert('Hapus Kategori', `Yakin ingin menghapus "${activeCategory.title}"?`, [
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.content}>
          <Text style={styles.resultCount}>{categories.length} kategori ujian</Text>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <View style={styles.badgeRow}>
                    <View style={styles.badge}>
                      <Text style={styles.badgeText}>{item.questionIds.length} soal</Text>
                    </View>
                  </View>
                </View>
              </TouchableOpacity>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{isEditMode ? 'Edit Kategori' : 'Tambah Kategori'}</Text>
        {isEditMode ? (
      ...
      <ScrollView contentContainerStyle={styles.formContainer} showsVerticalScrollIndicator={false}>
        ...
        <FieldGroup label="Jadwal">
          <TouchableOpacity style={styles.dateInput} onPress={() => setShowDatePicker(true)}>
            <Text style={schedule ? styles.dateInputText : styles.dateInputPlaceholder}>
          ...
          {showTimePicker && (
            <DateTimePicker value={scheduleDate} mode="time" display="default" onChange={onChangeTime} is24Hour />
          )}
        </FieldGroup>
        ...
        <TouchableOpacity style={styles.submitButton} onPress={handleSubmit}>
          <Text style={styles.submitButtonText}>{isEditMode ? 'Simpan Perubahan' : 'Tambah Kategori'}</Text>
        </TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  badge: { backgroundColor: '#61141A', borderRadius: 8, paddingHorizontal: 8, paddingVertical: 3 },
