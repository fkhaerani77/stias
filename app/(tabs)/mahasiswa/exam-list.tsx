...
import { ArrowLeft, Calendar, Clock, ListChecks, Lock } from 'lucide-react-native';
...
export default function ExamListScreen() {
  ...
  const { categories, getExamAccessStatus } = useExam();
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({ item }: any) => {
          const access = getExamAccessStatus(item);
          return (
            <TouchableOpacity
              style={styles.card}
              activeOpacity={0.8}
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                {!access.canStart && (
                  <Text style={[styles.metaText, { color: '#B08D8F', marginTop: 2 }]}>{access.message}</Text>
                )}
              </View>
            </TouchableOpacity>
          );
