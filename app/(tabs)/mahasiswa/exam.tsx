...
export default function ExamScreen() {
  ...
  useEffect(() => {
    ...
    if (!access.canStart) {
      Alert.alert("Akses Ditolak", access.message, [
        {
          text: "OK",
          onPress: () => router.replace("/mahasiswa/exam-list" as any),
        },
      ]);
    }
  }, [category?.id]);
  ...
  useEffect(() => {
    const backHandler = BackHandler.addEventListener(
      ...
      () => {
        ...
        Alert.alert(
          "Tidak Diizinkan",
          "Anda tidak dapat keluar dari ujian sebelum submit.",
        );
        return true;
      },
    );
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener("beforeRemove", (e: any) => {
      ...
      Alert.alert(
        "Tidak Diizinkan",
        "Anda tidak dapat meninggalkan layar ujian sebelum submit.",
      );
    });
    return unsubscribe;
  }, [navigation]);
  ...
  if (!category || QUESTIONS.length === 0) {
    return (
      <SafeAreaView style={styles.container}>
        ...
        >
          <Text
            style={{ color: "#61141A", textAlign: "center", marginBottom: 16 }}
          >
            Ujian tidak tersedia atau belum ada soal untuk kategori ini.
          </Text>
          <TouchableOpacity onPress={() => router.back()}>
            <Text style={{ color: "#61141A", fontWeight: "bold" }}>
              Kembali
            </Text>
          </TouchableOpacity>
        </View>
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.profileSection}>
          <Image
            source={{
              uri: "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=100&auto=format&fit=crop&q=60",
            }}
        ...
        <TouchableOpacity
          style={styles.questionListButton}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.modalTitle}>
              {isAllAnswered ? "Finish Exam?" : "Incomplete Answers"}
            </Text>
            <Text style={styles.modalDescription}>
              {isAllAnswered
                ? "Are you sure you want to submit? Changes cannot be undone."
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TouchableOpacity
              onPress={() => router.replace("/mahasiswa" as any)}
              style={{ marginTop: 20 }}
            >
